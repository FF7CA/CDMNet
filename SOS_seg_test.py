...
def get_args():
    ...
    arg("-c", "--config_path", type=Path, required=True, help="Path to  config", default='./config/mass/mvunet.py')
    arg("-o", "--output_path", type=Path, help="Path where to save resulting masks.", required=True, default='/root/results/mass/mvunet')
...
def main():
    ...
    print('F1:{:.4f}, mIOU:{:.4f}, OA:{:.4f}, P:{:.4f}, R:{:.4f}'.format(np.nanmean(f1_per_class[:-1]), np.nanmean(iou_per_class[:-1]), OA,
